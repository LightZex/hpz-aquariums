...
(function () {
  ...
  if (!container) return;
  if (typeof THREE === 'undefined') { console.warn('LiquidEther: THREE missing'); return; }
  if (reduce) return; // tôn trọng prefers-reduced-motion: giữ nền navy tĩnh

  const colors = ['#22b8e6', '#3fd0ff', '#aef3ff']; // xanh biển HPZ
  const mouseForce = 20;
  ...
  const bgVec4 = new THREE.Vector4(0, 0, 0, 0); // luôn transparent -> lộ nền navy
...
`;
    ...
    constructor(simProps) {
      super({
        material: { vertexShader: face_vert, fragmentShader: viscous_frag, uniforms: {
          ...
          velocity_new: { value: simProps.dst_.texture }, v: { value: simProps.viscous }, px: { value: simProps.cellScale }, dt: { value: simProps.dt }
  ...
  class Divergence extends ShaderPass {
    constructor(simProps) {
      super({ material: { vertexShader: face_vert, fragmentShader: divergence_frag, uniforms: {
        boundarySpace: { value: simProps.boundarySpace }, velocity: { value: simProps.src.texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt }
      } }, output: simProps.dst });
      this.init();
    }
  ...
  class Poisson extends ShaderPass {
    constructor(simProps) {
      super({ material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: {
        boundarySpace: { value: simProps.boundarySpace }, pressure: { value: simProps.dst_.texture }, divergence: { value: simProps.src.texture }, px: { value: simProps.cellScale }
      } }, output: simProps.dst, output0: simProps.dst_, output1: simProps.dst });
      this.init();
    }
    update({ iterations }) {
  ...
  class Pressure extends ShaderPass {
    constructor(simProps) {
      super({ material: { vertexShader: face_vert, fragmentShader: pressure_frag, uniforms: {
        boundarySpace: { value: simProps.boundarySpace }, pressure: { value: simProps.src_p.texture }, velocity: { value: simProps.src_v.texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt }
      } }, output: simProps.dst });
      this.init();
    }
  ...
  class Simulation {
    constructor(options) {
      this.options = {
        iterations_poisson: 32, iterations_viscous: 32, mouse_force: 20, resolution: 0.5, cursor_size: 100,
  ...
  class WebGLManager {
    ...
    loop() { if (!this.running) return; this.render(); rafRef = requestAnimationFrame(this._loop); }
  ...
  const ro = new ResizeObserver(() => {
    ...
    resizeRafRef = requestAnimationFrame(() => { if (webgl) webgl.resize(); });
  });
