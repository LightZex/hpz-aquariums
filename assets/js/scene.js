/* HPZ Aquaroom - Underwater 2D scene v2 (sprite-based, Kenney CC0 assets)
   Lặn sâu theo scroll + god rays nghiêng theo chuột + caustics gợn nước
   + bụi phù sa + cá né chuột + bầy cá nhỏ bơi theo đàn + ripple khi click
   + bong bóng mảnh + vầng sáng mặt nước loang. Tự giảm hiệu ứng trên mobile. */
(function () {
  ...
  // ---- Phân loại thiết bị để giảm hiệu ứng trên mobile yếu ----
  const isMobile = (window.innerWidth < 760) || /Mobi|Android/i.test(navigator.userAgent);
  const PERF = isMobile
    ? { fish: 10, flock: 0, dust: 55, bubbles: 24, caustics: false, big: false, rays: 6, dustW: 1 }
    : { fish: 16, flock: 3, dust: 130, bubbles: 44, caustics: true, big: true, rays: 9, dustW: 2 };
  ...
  // ---- Ripple khi click vào nước ----
  const ripples = [];
  window.addEventListener('pointerdown', function (e) {
    ripples.push({ x: e.clientX, y: e.clientY, r: 3, a: 0.5 });
  ...
  // ---- Bầy cá nhỏ theo đàn (điểm neo di chuyển + separation nhẹ) ----
  const flock = [];
  for (let g = 0; g < PERF.flock; g++) {
    const count = Math.floor(rand(9, 14));
    const hue = rand(185, 215);
  ...
  // ---- Sinh vật lớn thỉnh thoảng bơi ngang (desktop) ----
  let big = null;
  let bigTimer = 0;
  function spawnBig() {
    if (!PERF.big) return;
  ...
  for (let i = 0; i < PERF.rays; i++) {
    rays.push({
      ...
      amp: rand(30, 90), peak: rand(0.16, 0.32)
    });
  }

  // ---- Caustics: vệt sáng gợn sóng (chỉ desktop) ----
  const causticBands = [];
  if (PERF.caustics) {
    for (let i = 0; i < 3; i++) {
      causticBands.push({
        yBase: rand(0.05, 0.28),
        spd: rand(0.06, 0.14),
        phase: Math.random() * 6.283,
        alpha: rand(0.05, 0.1),
        wl1: rand(0.014, 0.026),
        wl2: rand(0.003, 0.007)
      });
    }
  }
  ...
  function frame(now) {
    ...
    // ---- Nền: lặn sâu theo scroll (gradient tối dần + xanh đậm dần) ----
    scrollP += (scrollTarget - scrollP) * Math.min(1, dt * 3);
    const cTop = [Math.round(lerp(16, 7, scrollP)), Math.round(lerp(80, 30, scrollP)), Math.round(lerp(126, 50, scrollP))];
    const cMid = [Math.round(lerp(9, 4, scrollP)), Math.round(lerp(51, 18, scrollP)), Math.round(lerp(82, 32, scrollP))];
    const cBot = [Math.round(lerp(5, 2, scrollP)), Math.round(lerp(29, 10, scrollP)), Math.round(lerp(49, 18, scrollP))];
    const g = ctx.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, 'rgb(' + cTop.join(',') + ')');
    g.addColorStop(0.45, 'rgb(' + cMid.join(',') + ')');
    g.addColorStop(1, 'rgb(' + cBot.join(',') + ')');
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, H);

    // ---- God rays (nghiêng nhẹ theo vị trí chuột) ----
    ...
    dust.forEach(function (p) {
      ...
      if (py < 0) py += H;
      ctx.globalAlpha = p.a * (0.5 + 0.5 * Math.sin(t * 0.6 + p.phase));
      ctx.fillRect(px, py, p.s * PERF.dustW, p.s * PERF.dustW);
    });
    ctx.restore();
    ...
    if (PERF.flock) {
      flock.forEach(function (b) {
        ...
        const ay = H * 0.32 + Math.sin(t * 0.47 + b.g) * H * 0.17;
        ...
        if (b.y > H * 0.72) { b.y = H * 0.72; b.vy = -Math.abs(b.vy); }
        // vẽ cá nhỏ: thân ellipse + đuôi, quay theo hướng
        const ang = Math.atan2(b.vy, b.vx);
        const tw = 15 + Math.sin(t * 8 + b.phase) * 2;
        const th = 6.5;
        ctx.save();
        ctx.translate(b.x, b.y);
        ctx.rotate(ang);
        ctx.globalAlpha = 0.72;
        ctx.fillStyle = 'hsl(' + b.hue.toFixed(0) + ',78%,70%)';
        ctx.beginPath();
        ctx.ellipse(0, 0, tw / 2, th / 2, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(-tw / 2, 0);
        ctx.lineTo(-tw / 2 - th * 0.75, -th / 2);
        ctx.lineTo(-tw / 2 - th * 0.75, th / 2);
        ctx.closePath();
        ctx.fill();
        ctx.restore();
    ...
    if (PERF.big) {
      if (!big) {
        bigTimer -= dt;
        if (bigTimer <= 0) { spawnBig(); bigTimer = rand(22, 45); }
      }
      if (big) {
        const w = 210;
        ...
        big.y += Math.sin(t * big.bob + big.phase) * 12 * dt;
        if ((big.dir > 0 && big.x > W + 300) || (big.dir < 0 && big.x < -300)) {
          big = null;
          bigTimer = rand(18, 40);
        } else {
          drawSprite('turtle2', big.x - w / 2, big.y - h / 2, w, h, big.dir < 0, 0.55);
        }
      }
    }
    ...
    // ---- Vầng sáng mặt nước loang động ----
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    const breathe = 0.5 + 0.5 * Math.sin(t * 0.35);
    const sg = ctx.createLinearGradient(0, 0, 0, H * 0.42);
    sg.addColorStop(0, 'rgba(175,232,255,' + (0.2 + 0.1 * breathe).toFixed(3) + ')');
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ctx.strokeStyle = 'rgba(215,245,255,' + (0.1 + i * 0.025).toFixed(3) + ')';
      ctx.lineWidth = 1.5;
      ctx.stroke();
    }
    ctx.restore();
